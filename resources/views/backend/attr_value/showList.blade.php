...
@section('content')
    <div class="box box-primary">
        ...
        <div class="mailbox-controls">
            <a href="{{asset('AttrValue/add')}}" class="label label-primary">添加属性值</a>
        </div>
                        ...
                        @else
                            @foreach(Session::get('button') as $but => $b_value)
                                @if($b_value['url'] == 'Attr/del')
                                    <a href="/Attr/del?id={{$value->value_id}}" class="label label-success">删除</a>
                                @elseif($b_value['url'] == 'Type/upd')
                                    <a href="/Attr/upd?id={{$value->value_id}}" class="label label-warning">修改</a>
                                @else
