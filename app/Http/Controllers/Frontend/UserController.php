...
class UserController extends Controller
{
        ...
        if($request->isMethod('post'))//判断是否是post请求，是post请求开始进行登录判断
        {
            ...
            if($result)
            {
                ...
                if($pwd == $result['u_password'])
                {
                    ...
                    if($login_log_info)
                    {
                        $request->session()->put('u_info',$result);
                        return redirect('/message')->
                        with(['message'=>'登录成功!','url' =>'/frontend/first/first', 'jumpTime'=>3,'status'=>true]);
                    }
                    else
                    {
                        return redirect('/message')->
                        with(['message'=>'日志错误!','url' =>'/frontend/user/login', 'jumpTime'=>3,'status'=>false]);
                    }
                ...
                {
                    return redirect('/message')->
                    with(['message'=>'密码错误!','url' =>'/frontend/user/login', 'jumpTime'=>3,'status'=>false]);
                }

            }else
            {
                return redirect('/message')->
                with(['message'=>'账号不存在!','url' =>'/frontend/user/login', 'jumpTime'=>3,'status'=>false]);
            }
        }
	...
	*/
    ...
    {
        $request->session()->pull('u_info');//删除session信息
        return redirect('/frontend/first/first');//返回前台首页
    }
        ...
        if($request->isMethod('post'))//判断是否是post请求，是post请求开始走入注册程序
        {
            ...
            if($info == 'tel')//如果是电话号码注册，走电话号码注册service类里面的电话号码注册方法
            {
                ...
                if($result)//判断返回值
                {
                    ...
                    $request->session()->put('u_info',$data);
                    return redirect('/message')->with(['message'=>'注册成功!','url' =>'/frontend/first/first', 'jumpTime'=>3,'status'=>true]);//返回成功信息并跳转到首页
                }
                else
                {
